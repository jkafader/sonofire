 ...
 * Uses diatonic harmony rules
 * @param {number} degree - Scale degree (1-7)
 * @param {string} mode - Mode hint ('major', 'minor', 'dorian', etc.)
 * @returns {string} Chord quality ('maj7', 'min7', '7', etc.)
 */
export function getChordQualityForDegreeInPool(degree, mode = 'major') {
    ...
    if (mode === 'minor' || mode === 'dorian' || mode === 'phrygian') {
        return minorQualities[degree] || 'maj7';
    } else {
        return majorQualities[degree] || 'maj7';
    }
}
