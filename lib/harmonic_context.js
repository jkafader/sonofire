...
class HarmonicContextService {
    ...
    /**
     * Convert legacy key/scale notation to pool/tonic notation
     * @param {string} key - Key name (C, D, E, F, G, A, B, etc.)
     * @param {string} scale - Scale type (major, minor, dorian, etc.)
     * @returns {Object} { poolKey, tonicName } - Pool/tonic equivalent
     */
    keyScaleToPoolTonic(key, scale = 'major') {
        // Map of major keys to their pool (based on circle of fifths)
        const majorKeyToPools = {
            'C': '0',
            'G': '1♯',
            'D': '2♯',
            'A': '3♯',
        ...
        // For modes, determine the pool based on the parent major key
        // Minor/Aeolian: 6th degree of major scale (3 semitones back on circle of fifths)
        // Dorian: 2nd degree (1 step back on circle)
        // Phrygian: 3rd degree (2 steps back)
        // Lydian: 4th degree (1 step forward)
        // Mixolydian: 5th degree (2 steps forward)
        // Locrian: 7th degree (5 steps back)

        const circleOfFifths = ['C', 'G', 'D', 'A', 'E', 'B', 'F♯/G♭', 'D♭', 'A♭', 'E♭', 'B♭', 'F'];
        const poolsInOrder = ['0', '1♯', '2♯', '3♯', '4♯', '5♯', '6♯', '5♭', '4♭', '3♭', '2♭', '1♭'];

        let poolKey;
        const normalizedKey = key.replace('♯', '#').replace('♭', 'b');

        if (scale === 'major') {
            poolKey = majorKeyToPools[normalizedKey] || majorKeyToPools[key] || '0';
        } else if (scale === 'minor' || scale === 'aeolian') {
            // Relative major is 3 semitones up (e.g., A minor → C major)
            const minorToMajor = {
                'A': 'C', 'E': 'G', 'B': 'D', 'F#': 'A', 'Gb': 'A',
                'C#': 'E', 'Db': 'E', 'G#': 'B', 'Ab': 'B',
                'D#': 'F#', 'Eb': 'Gb', 'A#': 'C#', 'Bb': 'Db',
                'F': 'Ab', 'C': 'Eb', 'G': 'Bb', 'D': 'F'
            };
            const relativeMajor = minorToMajor[normalizedKey] || minorToMajor[key] || 'C';
            poolKey = majorKeyToPools[relativeMajor] || '0';
        } else {
            // For other modes, use the same pool as if it were major for simplicity
    ...
    /**
     * Set the current key and scale (DEPRECATED - use setPoolAndTonic)
     * @deprecated Use setPoolAndTonic() instead
     * @param {string} key - Key name (C, D, E, F, G, A, B)
     * @param {string} scale - Scale type (major, minor, dorian, etc.)
     */
    setKey(key, scale = 'major') {
        console.warn('setKey() is deprecated. Converting to pool/tonic notation...');

        // Convert legacy notation to pool/tonic
        const { poolKey, tonicName } = this.keyScaleToPoolTonic(key, scale);

        console.log(`Converting "${key} ${scale}" → pool "${poolKey}" / tonic "${tonicName}"`);

        // Convert tonic name to MIDI note
        const tonicNote = this.noteNameToMIDI(tonicName, 4);

        // Call the new method
        this.setPoolAndTonic(poolKey, tonicNote, tonicName);
    }
     ...
     */
    setPoolAndTonic(poolKey, tonicNote, tonicName, scaleType = 'diatonic') {
        console.log(`HarmonicContext: setPoolAndTonic() called with poolKey="${poolKey}", tonicNote=${tonicNote}, tonicName="${tonicName}", scaleType="${scaleType}"`);
        ...
        console.log(`HarmonicContext: Got ${poolNotes.length} notes from pool "${poolKey}"`);

        // Update legacy scaleNotes for backward compatibility
        this.scaleNotes = poolNotes;

        // Publish to PubSub with pool/tonic/scaleType format
        console.log(`HarmonicContext: Publishing 'context:pool' event...`);
        ...
        console.log(`HarmonicContext: Published 'context:pool' event`);
     ...
     * @returns {Object} Current state with pool/tonic notation
     */
    getCurrentState() {
        return {
            poolKey: this.currentPoolKey || '0',
            tonicNote: this.currentTonicNote || 60,
            tonicName: this.currentTonicName || 'C',
            notes: this.scaleNotes || [],
            // Legacy fields for backward compatibility
            key: this.currentTonicName || 'C',
            scale: 'pool'
        };
    }
