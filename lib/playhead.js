...
export class Playhead {
    ...
    /**
     * Speed multipliers: /16, /12, /8, /4, /3, /2, 1, *2, *3, *4, *8, *12, *16
     * Encoded as numbers: 1/16 = 0.0625, *2 = 2, etc.
     * Base speed (1) = one cycle per measure
     */
    static SPEED_MULTIPLIERS = [
        { value: 1/16, label: '÷16 (very slow)', display: '÷16' },
        ...
        { value: 1, label: '1 (one cycle per measure)', display: '×1' },
        ...
        { value: 16, label: '×16 (very fast)', display: '×16' }
    ];
     ...
     * @returns {boolean} - True if playhead should advance, false otherwise
     */
    advance() {
        if (!this.enabled) return false;

        // Calculate effective ticks needed based on speed multiplier
        // speed = 1/16 → advance every 16 ticks
        // speed = 1/2 → advance every 2 ticks
        // speed = 1 → advance every tick
        // speed = 2 → advance twice per tick
        // speed = 4 → advance 4 times per tick
        const ticksNeeded = 1 / this.speed;

        this.tickCounter++;

        if (this.tickCounter >= ticksNeeded) {
            this.tickCounter = 0;
            return true; // Should advance
        }
