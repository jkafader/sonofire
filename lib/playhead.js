...
export class Playhead {
    constructor(visualizerId, config = {}) {
        ...
        this.layerId = config.layerId || null;  // Parent layer ID (playheads are children of layers)
    ...
    /**
     * Color palette for playheads (20 high-chroma, visually distinct colors)
     * Colors selected for maximum differentiation and vibrancy
     */
    ...
    /**
     * Generate next color from palette (cycles through palette)
     * @param {Array<Playhead>} existingPlayheads - Existing playheads to check colors against
     * @returns {string} - Hex color code
     */
    generateColor(existingPlayheads = []) {
        // Get colors already in use
        const usedColors = new Set(existingPlayheads.map(ph => ph.color));
        ...
        const index = existingPlayheads.length % Playhead.COLOR_PALETTE.length;
        return Playhead.COLOR_PALETTE[index];
    }
