...
export class WhipBinding {
    constructor(config = {}) {
        ...
        // Runtime state
        this.subscription = null; // PubSub subscription
        this.lastValue = null;
    }
    ...
    activate() {
        if (this.subscription) {
            console.warn(`WhipBinding ${this.id}: Already activated`);
            return;
        }
        ...
        // Mark as activated (PubSub doesn't return subscription handle)
        this.subscription = true;

        console.log(`WhipBinding ${this.id}: Activated (${this.sourceVisualizerId}:${this.sourcePlayheadId} → ${this.targetComponentId}:${this.targetParameterId})`);
    }
    ...
    deactivate() {
        ...
        // PubSub doesn't have an unsubscribe method, so we'll clear the topic callbacks
        // Note: This might affect other subscribers to the same topic
        // A better approach would be to add unsubscribe to PubSub, but for now we'll keep the subscription
        this.subscription = null;

        console.log(`WhipBinding ${this.id}: Deactivated`);
    }
    ...
    setEnabled(enabled) {
        ...
        console.log(`WhipBinding ${this.id}: ${enabled ? 'Enabled' : 'Disabled'}`);
    }
    ...
    setMappingFunction(mappingFunction, curve = 2) {
        ...
        console.log(`WhipBinding ${this.id}: Mapping function set to ${mappingFunction}`);
    }
