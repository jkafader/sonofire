...
const doSubscription = function(topic, callback, context, messageBus) {
    ...
    // Create a unique hash per instance by combining callback source + context identity
    // This prevents multiple instances with the same inherited callbacks from overwriting each other
    const callbackHash = callback.toLocaleString();

    // Add unique context identifier to the hash
    // Use a weak map to assign stable IDs to context objects
    if (!messageBus.contextIds) {
        messageBus.contextIds = new WeakMap();
        messageBus.nextContextId = 1;
    }
    if (!messageBus.contextIds.has(context)) {
        messageBus.contextIds.set(context, messageBus.nextContextId++);
    }
    const contextId = messageBus.contextIds.get(context);

    // Combine callback hash + context ID for unique per-instance subscription
    const hash = `${callbackHash}_ctx${contextId}`;

    // Enhanced logging for debugging subscription issues
    const contextName = context?.constructor?.name || 'unknown';

    if(messageBus.debug){
        console.debug(`[PubSub] Subscribing: topic="${topic}", context="${contextName}", contextId=${contextId}, hash="${hash.substring(0, 60)}..."`);
    ...
    if(!messageBus.topics[topic]){
        ...
        messageBus.debug && console.debug(`[PubSub] Created new topic "${topic}", subscriber count: 1`);
    } else {
        messageBus.topics[topic][hash] = {"callback": callback, "context":context };
        const count = Object.keys(messageBus.topics[topic]).length;
        messageBus.debug && console.debug(`[PubSub] Added to existing topic "${topic}", subscriber count: ${count}`);
    }
}
...
const doPublish = function(topic, eventData, messageBus) {
    ...
    if (!subscriberData) {
        messageBus.debug && console.log(`[PubSub] Publishing "${topic}" - no subscribers`);
        return;
    }
    var subscribers = Object.values(subscriberData);
    messageBus.debug && console.debug(`[PubSub] Publishing "${topic}" to ${subscribers.length} subscriber(s):`);
    for(var i=0; subscribers && i<subscribers.length; i++){
        const contextName = subscribers[i]["context"]?.constructor?.name || 'unknown';
        messageBus.debug && console.debug(`[PubSub]   → Calling subscriber #${i+1}: ${contextName}`);
...
const doUnsubscribe = (topic, callback, context, messageBus) => {
    if (!context) {
        context = this;
    }

    // Recreate the same hash that was used in doSubscription
    const callbackHash = callback.toLocaleString();

    if (!messageBus.contextIds || !messageBus.contextIds.has(context)) {
        if (messageBus.debug) {
            console.debug(`[PubSub] Unsubscribe failed: context not found for topic "${topic}"`);
        }
        return false;
    }
    ...
    if (messageBus.topics[topic] && messageBus.topics[topic][hash]) {
        delete messageBus.topics[topic][hash];

        // Clean up empty topic
        if (Object.keys(messageBus.topics[topic]).length === 0) {
            delete messageBus.topics[topic];
        }

        if (messageBus.debug) {
            const contextName = context?.constructor?.name || 'unknown';
            console.debug(`[PubSub] Unsubscribed: topic="${topic}", context="${contextName}", hash="${hash.substring(0, 60)}..."`);
        }
        return true;
    }

    if (messageBus.debug) {
        console.debug(`[PubSub] Unsubscribe failed: subscription not found for topic "${topic}"`);
    }
    return false;
}
    ...
    constructor(busScope, debug){
        ...
        this.global = {
            ...
            unsubscribe: function(topic, callback, context){
                return doUnsubscribe.call(self, topic, callback, context, self);
            },
...
PrivateMessageBus.prototype.unsubscribe = function(topic, callback, context){
    var messageBus = discoverBusAbove(context, this)
    return doUnsubscribe.call(messageBus, topic, callback, context, messageBus);
}
...
PrivateMessageBus.prototype.clearAllCallbacks = function(context) {
    var messageBus = discoverBusAbove(context, this);
    doClearAllCallbacks(messageBus);
};
