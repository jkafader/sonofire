...
class WhipManagerClass {
    ...
    registerBinding(binding) {
        // Check for duplicate binding (same source + target)
        const existingBinding = this.findBinding(
        ...
        if (existingBinding) {
            console.warn(`WhipManager: Binding already exists (${binding.sourceVisualizerId}/${binding.sourcePlayheadId} → ${binding.targetComponentId}/${binding.targetParameterId})`);
            return existingBinding; // Return existing binding instead of creating duplicate
        }
    ...
    /**
     * Find a binding by source and target
     * @param {string} visualizerId - Source visualizer ID
     * @param {string} playheadId - Source playhead ID
     * @param {string} componentId - Target component ID
     * @param {string} parameterId - Target parameter ID
     * @returns {WhipBinding|null}
     */
    findBinding(visualizerId, playheadId, componentId, parameterId) {
    ...
    /**
     * Update the color of all bindings from a specific playhead
     * @param {string} visualizerId
     * @param {string} playheadId
     * @param {string} newColor - New color value
     */
    updatePlayheadColor(visualizerId, playheadId, newColor) {
        const bindings = this.getBindingsForPlayhead(visualizerId, playheadId);
        ...
        if (bindings.length > 0) {
            ...
            // Publish color update event for target lights to sync
            PubSub.publish('whip:playhead:color:changed', {
                visualizerId,
                playheadId,
                color: newColor,
            ...
            console.log(`WhipManager: Updated color for ${bindings.length} binding(s) from playhead ${visualizerId}:${playheadId} to ${newColor}`);
        }
    }
