...
export const WhippableParametersMixin = (BaseClass) => class extends BaseClass {
    constructor() {
        super();
        this.whippableParameters = new Map(); // parameterId -> ParameterTarget (scalar bindings)
        this.contourParameters = new Map(); // parameterId -> ContourParameterTarget (contour bindings)
    ...
    registerWhippableParameter(parameterId, config = {}) {
        ...
        // Store element selector for dynamic querying
        if (config.elementSelector) {
            target.elementSelector = config.elementSelector;
        ...
        import('../whip_manager.js').then(({ WhipManager }) => {
            ...
            lights.forEach(lightElement => {
                ...
                if (bindings.length > 0) {
                    ...
                    // Mark as bound and disable the control element
                    this.boundParameters.add(parameterId);
                    const target = this.whippableParameters.get(parameterId);
                    if (target && target.elementSelector) {
                        const element = this.$(target.elementSelector);
                        if (element) {
                    ...
                    // No bindings - re-enable the control element
                    const target = this.whippableParameters.get(parameterId);
                    if (target && target.elementSelector) {
                        const element = this.$(target.elementSelector);
                        if (element) {
    ...
    setupWhipBindingSubscriptions() {
        ...
        PubSub.subscribe('whip:binding:register', (binding) => {
            if (binding.targetComponentId === componentId) {
                ...
                // Also sync all target lights to ensure consistent state
                requestAnimationFrame(() => {
                    this.syncTargetLightColors();
                });
            }
        ...
        // Listen for contour binding events
        this.setupContourBindingSubscriptions();
    }

    /**
     * Setup subscriptions for contour binding events
     */
    setupContourBindingSubscriptions() {
        const componentId = this.getComponentId();

        // Subscribe to contour binding registration
        PubSub.subscribe('contour-binding:register', (binding) => {
            if (binding.targetComponentId === componentId) {
                this.handleContourBindingAdded(binding.targetParameterId, binding);
            }
        }, this);

        // Subscribe to contour binding removal
        PubSub.subscribe('contour-binding:remove', (data) => {
            // Re-check all contour parameters to see if they still have bindings
            this.contourParameters.forEach((target, parameterId) => {
                import('../contour_binding_manager.js').then(({ contourBindingManager }) => {
                    const bindings = contourBindingManager.getBindingsForTarget(componentId, parameterId);
                    if (bindings.length === 0) {
                        this.handleContourBindingRemoved(parameterId);
                    }
                });
            });
        }, this);

        // Setup click handlers for contour target lights
        this.setupContourTargetLightClickHandler();
    }
    ...
    handleBindingAdded(parameterId, binding) {
        ...
        // Disable the control element (query it dynamically)
    ...
    handleBindingRemoved(parameterId) {
        ...
        // Re-enable the control element (query it dynamically)
        if (target.elementSelector) {
            const element = this.$(target.elementSelector);
            if (element) {
                element.disabled = false;
    ...
    /**
     * Setup handler for contour target light clicks (to remove contour bindings)
     */
    setupContourTargetLightClickHandler() {
        const componentId = this.getComponentId();

        // Delegate click handler for all contour target lights
        this.root.addEventListener('click', (event) => {
            const light = event.target.closest('.contour-target-light');
            if (!light) return;

            const targetId = light.dataset.contourTarget;
            if (!targetId) return;

            const [clickedComponentId, parameterId] = targetId.split(':');
            if (clickedComponentId !== componentId) return;

            // Get all contour bindings for this parameter
            import('../contour_binding_manager.js').then(({ contourBindingManager }) => {
                const bindings = contourBindingManager.getBindingsForTarget(componentId, parameterId);
                ...
                import('../layer_manager.js').then(({ layerManager }) => {
                    const bindingDescriptions = bindings.map(b => {
                        const layer = layerManager.getLayer(b.sourceLayerId);
                        const layerName = layer ? layer.name : 'Unknown Layer';
                        return `${layerName} → ${b.targetParameterId}`;
                    }).join('\n');

                    if (confirm(`Remove ${bindings.length} contour binding(s)?\n\n${bindingDescriptions}`)) {
                        // Remove all bindings for this parameter
                        bindings.forEach(binding => {
    ...
    /**
     * Handle a contour binding being added to one of our parameters
     * @param {string} parameterId
     * @param {Object} binding
     */
    handleContourBindingAdded(parameterId, binding) {
        const target = this.contourParameters.get(parameterId);
        if (!target) return;

        // Track bound layer
        if (!target.boundLayerIds.includes(binding.sourceLayerId)) {
            target.boundLayerIds.push(binding.sourceLayerId);
        }

        // Update contour target light color immediately
        const componentId = this.getComponentId();
        const targetLight = this.root.querySelector(`.contour-target-light[data-contour-target="${componentId}:${parameterId}"]`);
        if (targetLight && binding.color) {
            targetLight.style.background = binding.color;
            targetLight.style.opacity = '1.0';
            targetLight.style.boxShadow = `0 0 5px ${binding.color}`;
        }

        console.log(`Contour binding added: ${binding.getDisplayName()}`);
    }

    /**
     * Handle a contour binding being removed from one of our parameters
     * @param {string} parameterId
     */
    handleContourBindingRemoved(parameterId) {
        const target = this.contourParameters.get(parameterId);
        if (!target) return;

        // Clear bound layers
        target.boundLayerIds = [];

        // Reset contour target light color to default gray
        const componentId = this.getComponentId();
        const targetLight = this.root.querySelector(`.contour-target-light[data-contour-target="${componentId}:${parameterId}"]`);
        if (targetLight) {
            targetLight.style.background = '#666';
            targetLight.style.opacity = '0.7';
            targetLight.style.boxShadow = 'none';
        }

        console.log(`Contour bindings removed from: ${componentId}:${parameterId}`);
    }
    ...
    /**
     * Sync contour target light colors with existing bindings
     * Call this after rendering to update colors based on current bindings
     */
    syncContourTargetLightColors() {
        import('../contour_binding_manager.js').then(({ contourBindingManager }) => {
            import('../layer_manager.js').then(({ layerManager }) => {
                const componentId = this.getComponentId();

                // Query all contour target lights in this component
                const lights = this.root.querySelectorAll('.contour-target-light');

                lights.forEach(lightElement => {
                    const targetId = lightElement.dataset.contourTarget;
                    if (!targetId) return;

                    const [lightComponentId, parameterId] = targetId.split(':');
                    if (lightComponentId !== componentId) return;
                    ...
                    if (bindings.length > 0) {
                        ...
                        const binding = bindings[0];
                        if (binding.color) {
                            lightElement.style.background = binding.color;
                            lightElement.style.opacity = '1.0';
                            lightElement.style.boxShadow = `0 0 5px ${binding.color}`;
                        }

                        // Track bound layer in target
                        const target = this.contourParameters.get(parameterId);
                        if (target && !target.boundLayerIds.includes(binding.sourceLayerId)) {
                            target.boundLayerIds.push(binding.sourceLayerId);
                        }
                    } else {
                        // No bindings - reset to default
                        lightElement.style.background = '#666';
                        lightElement.style.opacity = '0.7';
                        lightElement.style.boxShadow = 'none';
                    }
                });
            });
        }).catch(err => {
            console.warn('Could not sync contour target light colors:', err);
        });
    }

    /**
     * Register a contour parameter (for layer-to-planner bindings)
     * @param {string} parameterId - Unique parameter identifier
     * @param {Object} config - Parameter configuration
     *   - label: Display label
     *   - parameterType: Must be 'contour'
     *   - contourType: 'rhythm', 'melody', 'envelope', or 'generic'
     *   - setter: Function to call when contour changes
     */
    registerContourParameter(parameterId, config = {}) {
        const componentId = this.getComponentId();
        const target = new ContourParameterTarget(parameterId, config, this);

        this.contourParameters.set(parameterId, target);

        // Setup contour binding subscriptions on first parameter registration
        if (!this.whipBindingSubscriptionsSetup) {
            this.setupWhipBindingSubscriptions();
            this.whipBindingSubscriptionsSetup = true;
        }
        ...
        // Publish registration event
        PubSub.publish('contour-parameter:target:register', {
            componentId: componentId,
            parameterId: parameterId,
            config: config
        });

        console.log(`Contour parameter registered: ${componentId}:${parameterId}`);

        return target;
    }

    /**
     * Unregister a contour parameter
     * @param {string} parameterId
     */
    unregisterContourParameter(parameterId) {
        const target = this.contourParameters.get(parameterId);
        if (!target) return;

        // Publish unregistration event
        PubSub.publish('contour-parameter:target:unregister', {
            componentId: this.getComponentId(),
            parameterId: parameterId
        });

        this.contourParameters.delete(parameterId);
    }

    /**
     * Set a contour parameter value (called by contour bindings)
     * @param {string} parameterId
     * @param {Object} contourData - Contour data from layer
     */
    setContourValue(parameterId, contourData) {
        const target = this.contourParameters.get(parameterId);
        if (!target) {
            console.warn(`Contour parameter not found: ${parameterId}`);
            return;
        }

        // Call setter with contour data
        if (target.setter) {
            target.setter.call(this, contourData);
        }

        // Publish change event
        PubSub.publish(`contour-parameter:${this.getComponentId()}:${parameterId}:changed`, {
            contour: contourData
        });
    }

    /**
     * Generate HTML for a contour target light (square, for layer bindings)
     * @param {string} parameterId - The parameter ID
     * @returns {string} HTML for the contour-target-light element
     */
    getContourTargetHTML(parameterId) {
        const componentId = this.getComponentId();
        const target = this.contourParameters.get(parameterId);
        const label = target ? target.label : parameterId;

        return `<div class="contour-target-light"
                     data-contour-target="${componentId}:${parameterId}"
                     style="
                         width: 12px;
                         height: 12px;
                         background: #666;
    ...
    disconnectedCallback() {
        // Remove all scalar target lights
        this.whippableParameters.forEach((target, parameterId) => {
            this.unregisterWhippableParameter(parameterId);
        });

        // Remove all contour target lights
        this.contourParameters.forEach((target, parameterId) => {
            this.unregisterContourParameter(parameterId);
        });
...
/**
 * ContourParameterTarget - Represents a contour parameter target
 */
class ContourParameterTarget {
    constructor(parameterId, config, component) {
        this.parameterId = parameterId;
        this.componentId = component.getComponentId();
        this.label = config.label || parameterId;
        this.setter = config.setter;
        this.contourType = config.contourType || 'generic';  // 'rhythm', 'melody', 'envelope'

        this.boundLayerIds = [];  // Track bound layers
    }

    /**
     * Create square target light element
     */
    createTargetLight() {
        const light = document.createElement('div');
        light.className = 'contour-target-light';
        light.dataset.contourTarget = `${this.componentId}:${this.parameterId}`;
        light.style.width = '12px';
        light.style.height = '12px';
        light.style.background = '#666';
        light.style.border = '1px solid #fff';
        light.style.display = 'inline-block';
        light.style.marginLeft = '5px';
        light.style.cursor = 'pointer';
        light.style.opacity = '0.7';
        light.title = `Contour target: ${this.label}`;
