...
export const PLAYHEAD_SIDEBAR_WIDTH = 110;
...
export function PlayheadsMixin(BaseClass) {
    return class extends BaseClass {
        ...
        addPlayhead(config = {}) {
            ...
            // Auto-assign layer if not provided
            if (!config.layerId) {
                const layers = layerManager.getLayersForVisualizer(visualizerId);

                // Find selected layer, or fall back to first data layer
                const selectedLayer = layers.find(l => l.selected);
                const dataLayer = layers.find(l => l.type === 'data');
                const targetLayer = selectedLayer || dataLayer || layers[0];

                if (targetLayer) {
                    config.layerId = targetLayer.id;
                }
            }

            // Generate color if not provided (cycle through palette based on existing playheads)
            if (!config.color) {
                const usedColors = new Set(this.playheads.map(ph => ph.color));
                for (const color of Playhead.COLOR_PALETTE) {
                ...
                if (!config.color) {
                    const index = this.playheads.length % Playhead.COLOR_PALETTE.length;
                    config.color = Playhead.COLOR_PALETTE[index];
                }
            }
         ...
         * If no playheads exist, creates one default playhead
         */
        restorePlayheads() {
            const visualizerId = this.getVisualizerId();
            const lastState = this.getLastValue(`visualizer:${visualizerId}:playheads`);

            console.log(`[Playheads] restorePlayheads() called for ${visualizerId}, current count: ${this.playheads.length}, saved count: ${lastState?.playheads?.length || 0}`);

            if (!lastState || !lastState.playheads || lastState.playheads.length === 0) {
                console.log(`No saved playheads for ${visualizerId}, creating default playhead`);

                // Create one default playhead
                this.addPlayhead({});
                return;
            }
            ...
            console.log(`[Playheads] Restored ${this.playheads.length} playheads for ${visualizerId}`);

            // Migrate playheads without layerId (assign to data layer)
            const layers = layerManager.getLayersForVisualizer(visualizerId);
            const dataLayer = layers.find(l => l.type === 'data');

            this.playheads.forEach(playhead => {
                if (!playhead.layerId && dataLayer) {
                    playhead.layerId = dataLayer.id;
                    console.log(`Migrated playhead ${playhead.id} to layer ${dataLayer.name}`);
                }
            });
        ...
        renderPlayheadControls() {
            ...
            // Create container if it doesn't exist OR if it's been removed from DOM
            if (!this.playheadControlsContainer || !this.contains(this.playheadControlsContainer)) {
                ...
                // Set to fixed value to avoid accumulation on multiple renders
                if (!this._playheadPaddingAdded) {
                    const currentPadding = this.style.paddingLeft;
                    const currentPaddingValue = currentPadding ? parseInt(currentPadding) : 0;
                    this._originalPaddingLeft = currentPaddingValue;
                    this._playheadPaddingAdded = true;
                }
                //this.style.paddingLeft = `${this._originalPaddingLeft + PLAYHEAD_SIDEBAR_WIDTH}px`;

                // Insert at the beginning of the visualizer
                this.insertBefore(this.playheadControlsContainer, this.firstChild);
            }

            // Filter playheads by selected layer
            const layers = layerManager.getLayersForVisualizer(visualizerId);
            const selectedLayer = layers.find(l => l.selected);
            const filteredPlayheads = selectedLayer ?
                this.playheads.filter(ph => ph.layerId === selectedLayer.id) :
                this.playheads;

            // Get selected layer name for display
            const layerName = selectedLayer ? selectedLayer.name : 'All Layers';

            // Render compact sidebar HTML
            this.playheadControlsContainer.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; flex-shrink: 0; padding: 12px 0 0 4px;">
                    <strong>Playheads</strong>
                </div>
                <div style="padding: 4px; color: #888; font-size: 10px; border-bottom: 1px solid #555; margin-bottom: 8px;">
                    Layer: <span style="color: ${selectedLayer?.color || '#d4d4d4'};">${layerName}</span>
                </div>
                <div id="playhead-list" style="flex: 1; overflow-y: auto;">
                    ${filteredPlayheads.length === 0 ?
                        '<div style="padding: 10px; color: #888; font-size: 11px; text-align: center;">No playheads for this layer</div>' :
                        filteredPlayheads.map((ph, index) => this.renderPlayheadItem(ph, index)).join('')
                    }
                </div>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div style="display: flex; align-items: center; gap: 4px;">
                            ...
                            " class='playhead-indicator'
                                data-playhead-id="${playhead.id}"
                                data-visualizer-id="${visualizerId}"
                                data-color="${playhead.color}"
                                title="Drag to create whip binding">
                                <input type="color" id='the-picker' class="color-picker" data-playhead-id="${playhead.id}" value="${playhead.color}" style="
                                    ...
                                    padding: 0;
                                    opacity: 0;
                                " title="Change color (right-click)">
                            </div>
        ...
        setupPlayheadEventHandlers() {
            ...
            // Playhead indicators - set up whip drag and color picker
            this.playheadControlsContainer.querySelectorAll('.playhead-indicator').forEach(indicator => {
                const playheadId = indicator.dataset.playheadId;
                const playhead = this.getPlayhead(playheadId);
                if (!playhead) return;

                // Whip drag on left-click drag
                indicator.onmousedown = (event) => {
                    // Only left-click
                    if (event.button !== 0) return;

                    // Import WhipDragHandler dynamically
                    import('../whip_drag_handler.js').then(({ WhipDragHandler }) => {
                        WhipDragHandler.startDrag(event, {
                            visualizerId: indicator.dataset.visualizerId,
                            playheadId: playhead.id,
                            playhead: playhead,
                            color: playhead.color,
                            sourceElement: indicator
                        });
                    });
                ...
                indicator.oncontextmenu = (event) => {
                    event.preventDefault();
                    const picker = indicator.querySelector('.color-picker');
                    if (picker) {
                        picker.click();
                    }
                };
        ...
        handleSetPlayheadColor(playheadId, color) {
            ...
            // Notify WhipManager to update binding colors
            import('../whip_manager.js').then(({ WhipManager }) => {
                WhipManager.updatePlayheadColor(this.getComponentId(), playheadId, color);
            });
        }
    };
}
